...
/*
	Cette fonction évite que les philosophes ne prennent pas les mêmes fourchettes en même temps,
	évitant ainsi un blocage ou une attente infinie qui empêche les philosophes de manger et évite
	que le programme ne tombe en panne.
*/
void    fork_choice(t_table *table, t_philo *philo)
{
    int left_fork = philo->num - 1;
    int right_fork = philo->num % table->philo_count;

    philo->fork_one = left_fork < right_fork ? &table->forks[left_fork] : &table->forks[right_fork];
    philo->fork_two = left_fork < right_fork ? &table->forks[right_fork] : &table->forks[left_fork];
}
...
// Cette fonction initialise les mutex pour chaque philosophe dans la structure de la table
